...
3. **Descubra o time do Linear.** Rode `list_teams`. Se houver mais de um time, pergunte em qual criar. Se houver só um, use ele sem perguntar.
4. **Monte o preview e confirme.** Mostre no chat: nome do projeto, a descrição completa e a lista de títulos de issue (um por fluxo). Só crie no Linear depois de ela confirmar.
5. **Crie o projeto** com `save_project`: `name`, `description` (no formato `/task`) e `setTeams` com o time.
6. **Crie as issues** com `save_issue`: `title` = nome do fluxo, `team` e `project` = o projeto criado. Uma issue por fluxo, sem corpo obrigatório.
7. **Devolva os links.** Liste o projeto e as issues criadas com seus identificadores/URLs.
...
## Dados
[evidência quantitativa ou qualitativa que embasa o problema; se não houver, `> [a definir: levantar dado de X]`]
...
Regras rápidas das seções: **Oportunidade** é o problema numa frase, não a solução. **Contexto** é concreto (telas, perfis, onde dói). **Consequências** e **Impacto Almejado** em bullets de uma ideia cada. **Dados** é evidência real; sem ela, `> [a definir: ...]`. **Solução** em duas frases, sem virar especificação. **Hipótese** sempre no formato `Se fizermos ___, então veremos ___`. **Comportamento Esperado** é o coração: para cada fluxo, um `### Nome do Fluxo`, passos numerados na ótica do usuário, e um `### Regras de negócio` logo embaixo. Repita o par (fluxo → regras) para cada fluxo. (Detalhe completo na skill `/task`.)

## Ferramentas do Linear

- `list_teams` — descobrir o(s) time(s). Rode antes de criar qualquer coisa.
- `list_projects` — checar se já existe projeto com o mesmo nome antes de criar, pra evitar duplicata.
- `save_project` — cria o projeto. Campos: `name`, `description` (markdown, com quebras de linha e caracteres literais, sem escapar), `setTeams: ["<time>"]`. Opcionais úteis: `summary` (resumo curto até 255 chars), `lead: "me"`, `priority` (0=None, 1=Urgent, 2=High, 3=Medium, 4=Low).
- `save_issue` — cria cada issue. Campos: `title`, `team` e `project` (nome, id ou slug do projeto criado). Nada além disso é obrigatório.
