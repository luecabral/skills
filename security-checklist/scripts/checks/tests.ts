...
export async function check(projectRoot: string, _context?: CheckContext): Promise<CategoryResult> {
  ...
  const crossTenantTests = await grepInFiles(
    testFiles,
    /cross.workspace|cross.tenant|isolation|cross_workspace|crossWorkspace/i,
  );
  items.push({
    id: 'M1',
  ...
  const privilegeTestsByName = testFiles.filter(
    (f) => /privilege|escalation|escalada/i.test(f),
  );
  const privilegeTests = [
    ...privilegeTestsByName.map((f) => ({ file: f, line: 0, text: 'filename match' })),
    ...(await grepInFiles(
      testFiles,
      /privilege.escalation|escalation|unauthorized.*access|should.*return.*403|403.*forbidden|access.*denied|NÃO pode|não.*permitido/i,
    )),
  ];
  items.push({
    id: 'M2',
  ...
  const passwordTests = await grepInFiles(
    testFiles,
    /password.*test|test.*password|senha.*test|test.*senha|weak.*password|strong.*password|zxcvbn/i,
  );
  items.push({
    id: 'M3',
  ...
  const rateLimitTests = await grepInFiles(
    testFiles,
    /rate.limit|rateLimit|too.many.requests|429|throttle/i,
  );
  items.push({
    id: 'M4',
  ...
  const cryptoTests = await grepInFiles(
    testFiles,
    /crypt|encrypt|decrypt|argon2|hash.*password|password.*hash|kdf|cipher/i,
  );
  items.push({
    id: 'M5',
