...
function formatScopeLabel(scope) {
    if (scope === 'stack-specific')
        return `${DIM}[stack-specific]${RESET}`;
    if (scope === 'generic')
        return `${DIM}[generic]${RESET}`;
    return '';
}
...
function formatTrend(report, lastAudit) {
    if (!lastAudit)
        return '';
...
export function formatTerminal(report, lastAudit) {
    const lines = [];
    lines.push(`\n${BOLD}${LINE}${RESET}`);
    ...
    lines.push(`${DIM}  Stack: eco=${report.stack.ecosystem}, pm=${report.stack.packageManager}, rails=${report.stack.frameworks.rails ? 'sim' : 'não'}, nextjs=${report.stack.frameworks.nextjs ? 'sim' : 'não'}, postgres=${report.stack.services.postgres ? 'sim' : 'não'}, redis=${report.stack.services.redis ? 'sim' : 'não'}, sidekiq=${report.stack.services.sidekiq ? 'sim' : 'não'}, hotwire=${report.stack.frameworks.hotwire ? 'sim' : 'não'}${RESET}`);
    ...
    for (const category of report.categories) {
        ...
        lines.push(`${BOLD}${category.id}. ${category.name}${RESET} ${formatLayerLabel(category.layer)} ${formatScopeLabel(category.scope)}  ${categoryColor}${categoryScore}${RESET}`);
        lines.push(DASH);
        for (const item of category.items) {
            ...
            lines.push(`  ${statusIcon} ${sevIcon} ${statusColor}${item.description}${RESET}${itemScopeLabel}`);
...
export function formatNAItems(naItems) {
    if (naItems.length === 0)
        return '';
