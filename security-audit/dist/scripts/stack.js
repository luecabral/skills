...
function hasDependency(pkg, name) {
    if (!pkg)
        return false;
    return Boolean((pkg.dependencies && name in pkg.dependencies) ||
...
function hasGem(gemfile, gemName) {
    if (!gemfile)
        return false;
    const pattern = new RegExp(`^\\s*gem\\s+['"]${gemName}['"]`, 'm');
    return pattern.test(gemfile);
}
function detectEcosystem(hasNode, hasRuby) {
    if (hasNode && hasRuby)
        return 'mixed';
    if (hasNode)
        return 'node';
...
export async function detectStack(projectRoot) {
    ...
    const hasNextjs = hasDependency(packageJson, 'next') || nextConfigFiles.length > 0 || hasNextRouting;
    ...
    const hasSupabaseDeps = hasDependency(packageJson, '@supabase/supabase-js') || hasDependency(packageJson, '@supabase/ssr');
    ...
    const hasTailwindConfig = (await fileExists(join(projectRoot, 'tailwind.config.js'))) ||
    ...
    const hasEsbuildConfig = (await fileExists(join(projectRoot, 'esbuild.config.js'))) ||
