...
import { fileExists, globFiles, grepInFiles, fileContains, readJsonFile, } from '../utils.js';
export async function check(projectRoot, context) {
    ...
    const rateLimitMatches = await grepInFiles(routeFiles, /rate.?limit|rateLimit|rateLimiter|upstash|redis.*rate/i);
    items.push({
        id: 'B1',
    ...
    items.push({
        ...
        status: routeFiles.length === 0 ? 'warn' : perEndpointRL.length >= 2 ? 'pass' : perEndpointRL.length === 1 ? 'warn' : 'fail',
        severity: 'high',
        detail: routeFiles.length === 0
            ? 'Nenhum endpoint HTTP detectado no padrão analisado'
    ...
    const hasZxcvbn = (pkg?.dependencies && 'zxcvbn' in pkg.dependencies) ||
    ...
    const hasMiddleware = (await fileExists(middlewarePath)) || (await fileExists(middlewareSrcPath)) ||
    ...
    const middlewareFile = (await fileExists(middlewarePath))
        ...
        : (await fileExists(proxyPath))
            ...
            : (await fileExists(proxySrcPath))
                ? proxySrcPath
                : middlewareSrcPath;
    const middlewareHasAuth = hasMiddleware &&
    ...
    const serviceClientMatches = await grepInFiles(allTsFiles, /createServiceClient|service_role|supabaseAdmin|createAdminClient/i);
    const anonClientMatches = await grepInFiles(allTsFiles, /createClient|createBrowserClient|createServerClient/i);
    items.push({
        id: 'B5',
    ...
    const supabaseSsrMatches = await grepInFiles(allTsFiles, /@supabase\/ssr|createServerClient|createBrowserClient/i);
    ...
    const cookieFlagsOk = usesSupabaseSsr ||
    ...
    const authErrorMatches = await grepInFiles(routeFiles, /user.not.found|email.not.found|email.*not.*exist|usuário.*não.*encontrado/i);
    const genericAuthError = await grepInFiles(routeFiles, /Invalid credentials|Credenciais inválidas|email.*or.*password|email.*ou.*senha/i);
    items.push({
        id: 'B7',
        description: 'Prevenção de enumeração de usuários',
        status: authErrorMatches.length > 0 ? 'fail' : genericAuthError.length > 0 ? 'pass' : 'warn',
        severity: 'medium',
        detail: authErrorMatches.length > 0
