...
import { fileExists, grepInFiles, globFiles } from '../utils.js';
export async function check(projectRoot, _context) {
    ...
    const permissionsPolicyMatches = await grepInFiles(headerFiles, /Permissions-Policy|permissionsPolicy|Feature-Policy/i);
    items.push({
        id: 'D6',
    ...
    const csrfOriginMatches = await grepInFiles(headerFiles, /request\.headers\.get\(['"]origin|origin.*===|checkOrigin|validateOrigin|csrf/i);
    ...
    const sanitizeMatches = await grepInFiles(allTsFiles, /DOMPurify|sanitize|xss|escapeHtml|dangerouslySetInnerHTML.*sanitize/i);
