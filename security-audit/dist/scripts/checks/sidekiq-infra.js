...
export async function check(projectRoot, _context) {
    ...
    const idempotencyHints = await grepInFiles(workerFiles, /idempot|dedup|unique_job|sidekiq-unique-jobs|lock:\s*:until_executed/i);
    items.push({
        id: 'S3',
    ...
    const sensitiveArgs = await grepInFiles(workerFiles, /perform_async\(|perform_later\(|set\(.+perform_async/i);
