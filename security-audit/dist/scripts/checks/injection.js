...
export async function check(projectRoot, context) {
    ...
    const [evalMatches, newFunctionMatches, execMatches, execWithInterpolation, sqlTemplateMatchesRaw, parameterizedClientMatches, rawSqlMatches] = await Promise.all([
    ...
    const sqlTemplateMatches = sqlTemplateMatchesRaw.filter((m) => {
        ...
        if (/\.from\(|\.rpc\(|supabase\./i.test(m.text))
            return false;
        // Log/console messages
        if (/console\.|logger\.|log\(/i.test(m.text))
            return false;
        // Comments
        if (/^\s*(\/\/|\/\*|\*)/.test(m.text))
            return false;
