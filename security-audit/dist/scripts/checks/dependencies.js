...
function fromAdvisories(advisories) {
    if (!advisories)
        return null;
    let critical = 0;
    let high = 0;
...
function commandsForPackageManager(pm) {
    if (pm === 'pnpm')
        return ['pnpm audit --audit-level high --json', 'npm audit --audit-level=high --json'];
    if (pm === 'yarn') {
        return [
...
function remediationForPackageManager(pm) {
    if (pm === 'pnpm')
        return 'Execute `pnpm audit --audit-level high` e `pnpm audit --fix` (ou aplique updates manuais)';
    if (pm === 'yarn')
        return 'Execute `yarn npm audit` (Berry) ou `yarn audit` (Classic) e atualize dependências vulneráveis';
    return 'Execute `npm audit` e `npm audit fix`, ou atualize manualmente as dependências vulneráveis';
}
function parseAuditFromCommandOutput(output) {
...
export async function check(projectRoot, context) {
    ...
    const hasLockfile = (await fileExists(lockfilePath)) ||
    ...
    if (hasRuby) {
        let rubyAuditStatus = 'warn';
        let rubyAuditDetail = 'bundle-audit nao executado';
        let rubyAuditRemediation = 'Execute `bundle-audit check --update` (ou `bundle exec bundle-audit check --update`) e aplique os upgrades recomendados';
        let rubyCommandUsed;
        const rubyCommands = [
    ...
    if (await fileExists(workflowDir)) {
        ...
        const [auditWorkflow, cronSchedule] = await Promise.all([
            grepInFiles(workflowFiles, /npm audit|pnpm audit|yarn audit|bundle-audit|brakeman|audit.*dependencies|snyk|dependabot/i, context),
