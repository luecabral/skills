...
export async function check(projectRoot, _context) {
    ...
    const uuidValidationMatches = await grepInFiles(allTsFiles, /[0-9a-f]{8}-[0-9a-f]{4}.*regex|uuid.*regex|regex.*uuid|z\.string\(\)\.uuid|isUUID|validate.*uuid|uuid.*valid/i);
    const uuidRegexLiteral = await grepInFiles(allTsFiles, /\/\^?\[0-9a-f\]|\/\^[0-9a-fA-F]{8}|['"]\^?[0-9a-f]{8}-/i);
    ...
    const clientOnlyValidation = await grepInFiles(await globFiles(projectRoot, ['app/**/*.tsx', 'components/**/*.tsx', 'src/**/*.tsx']), /onSubmit.*validate|handleSubmit.*validate|client.side.*valid/i);
