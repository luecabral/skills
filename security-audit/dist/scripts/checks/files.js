...
export async function check(projectRoot, context) {
    ...
    const pathValidationBeforeStorage = await grepInFiles(serverFiles, /uuid.*valid|valid.*uuid|z\.string\(\)\.uuid|isUUID|validatePath|sanitizePath/i);
    items.push({
        id: 'G1',
        description: 'Validação de path/UUID antes de operações de storage',
        status: storageOpFiles.length === 0
            ? 'warn'
            : pathValidationBeforeStorage.length > 0
                ? 'pass'
                : 'fail',
        severity: 'high',
        detail: storageOpFiles.length === 0
    ...
    const dotdotInPaths = dotdotInPathsRaw.filter((m) => {
        if (/^\s*(import\s|from\s+['"]|require\s*\()/.test(m.text))
            return false;
        if (/^\s*(\/\/|\/\*|\*)/.test(m.text))
            return false;
        return true;
    ...
    const fileTypeValidation = await grepInFiles(serverFiles, /allowedTypes|allowedMimetypes|whitelist.*type|type.*whitelist|\.includes\(.*mime|file\.type.*===|mimetype.*startsWith/i);
    items.push({
        id: 'G3',
        description: 'Whitelist de extensões/tipos para upload',
        status: uploadHandlers.length === 0
            ? 'warn'
            : fileTypeValidation.length > 0
                ? 'pass'
                : 'fail',
        severity: 'high',
        detail: uploadHandlers.length === 0
    ...
    const fileSizeValidation = await grepInFiles(serverFiles, /maxSize|max_size|MAX_SIZE|file\.size|fileSize|size.*limit|limit.*size/i);
    items.push({
        id: 'G4',
    ...
    const sourceMapConfig = await grepInFiles(nextConfigFiles, /productionBrowserSourceMaps|sourceMaps|source.?map/i);
    const sourceMapsDisabled = await grepInFiles(nextConfigFiles, /productionBrowserSourceMaps\s*:\s*false|sourceMaps.*false/i);
    items.push({
        id: 'G6',
        description: 'Source maps desabilitados em produção (Next.js)',
        scope: 'stack-specific',
        status: !isNextProject
            ? 'skip'
