...
export async function check(projectRoot, context) {
    ...
    const sourceFiles = allTsFiles.filter((f) => !f.includes('.test.') && !f.includes('.spec.') && !f.includes('__tests__'));
    ...
    const isLikelyFalsePositive = (text) => {
        // process.env references
        if (/process\.env/.test(text))
            return true;
        // import/require statements
        if (/import\s|require\(/.test(text))
            return true;
        // DB operations with onConflict/conflict resolution
        if (/onConflict|upsert|\.from\(/.test(text))
            return true;
        // Type annotations
        if (/:\s*(string|number|boolean|Record|Partial)/.test(text))
            return true;
        return false;
    };
    ...
    const [memzeroMatches, argon2InCode, hasWeakHash, nextPublicSecretsRaw, envFiles, localStorageSecrets] = await Promise.all([
    ...
    const hasArgon2Pkg = (pkg?.dependencies && ('argon2' in pkg.dependencies || 'argon2id' in pkg.dependencies)) ||
    ...
    items.push({
        ...
        severity: 'critical',
        detail: nextPublicSecrets.length > 0 || nextPublicInEnv.length > 0
