...
import { loadConfig, createDefaultConfig, saveAuditResult, getLastAudit, isItemExcluded, } from './memory.js';
...
function calculateBreakdown(items) {
...
async function configureScan(projectRoot) {
...
async function main() {
    ...
    if (!json && !sarif) {
        ...
        console.log(`Stack detectado: eco=${stack.ecosystem}, pm=${stack.packageManager}, rails=${stack.frameworks.rails ? 'sim' : 'não'}, next=${stack.frameworks.nextjs ? 'sim' : 'não'}, postgres=${stack.services.postgres ? 'sim' : 'não'}, redis=${stack.services.redis ? 'sim' : 'não'}, sidekiq=${stack.services.sidekiq ? 'sim' : 'não'}`);
    ...
    for (const category of categoryResults) {
        ...
        for (const item of category.items) {
            ...
            if (excluded) {
                ...
                // Mark as skip
                filteredItems.push({ ...item, status: 'skip' });
            }
            else {
    ...
    const criticalFailures = scorableItems.filter((i) => i.status === 'fail' && (i.severity === 'critical' || i.severity === 'high'));
    ...
    const hasFailures = scorableItems.some((i) => i.status === 'fail' && severityLevels[i.severity] >= failOnLevel);
